        ...
        case None =>
          nodeStorage.get(id) match {
            case Some(x) =>
              ...
              x
            case None =>
              throw MPTException(s"Node not found ${khipu.toHexString(nodeId)}, trie is inconsistent")
          }
        case Some(Deleted(_)) =>
          throw MPTException(s"Node has been deleted ${khipu.toHexString(nodeId)}, trie is inconsistent")
      }
    }
