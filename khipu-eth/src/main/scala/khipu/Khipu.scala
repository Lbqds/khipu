...
object Khipu {
  ...
  def main(args: Array[String]) {
    ...
    //new khipu.store.HashedKeyValueStorage(serviceBoard.blockchain)(system).loadFromKafka()
  ...
  def startOutgoing_test(uri: URI) {
    ...
    val peer = new OutgoingPeer(Peer.peerId(uri), uri)
  ...
  def startSyncService() {
    SyncService.start(system, Some("entity"),
      serviceBoard.ledger,
      serviceBoard.validators,
      serviceBoard.blockchain,
      serviceBoard.storages.appStateStorage,
      serviceBoard.storages.fastSyncStateStorage,
      serviceBoard.miningConfig,
      serviceBoard.peerConfiguration)
