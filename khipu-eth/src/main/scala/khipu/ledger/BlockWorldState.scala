...
final class BlockWorldState private (
  ...
  private[ledger] def merge(laster: BlockWorldState): Either[Map[RaceCondition, Set[Address]], BlockWorldState] = {
    val raceCondiftions = this.raceConditions.foldLeft(Map[RaceCondition, Set[Address]]()) {
      case (acc, (OnAccount, addresses)) => acc + (OnAccount -> addresses.filter(laster.trieAccounts.logs.contains))
      case (acc, (OnStorage, addresses)) => acc + (OnStorage -> addresses.filter(laster.trieStorages.contains))
      case (acc, (OnCode, addresses))    => acc + (OnCode -> addresses.filter(laster.codes.contains))
    ...
    if (raceCondiftions.isEmpty) {
      ...
      toMerge.touchedAddresses ++= laster.touchedAddresses
      //mergeAccountTrieAccount_simple(toMerge, that)
      toMerge.mergeAccountTrieAccount(laster).mergeTrieStorage(laster).mergeCode(laster).mergeRaceConditions(laster)
      Right(toMerge)
    } else {
